...
export default function ReferralsPage() {
  ...
  const referralLink = `https://t.me/your_bot?start=ref${user?.id || "123456"}`;
  ...
  const totalEarned = referrals.reduce((sum, ref) => sum + ref.bonusEarned, 0);
  ...
  return (
    ...
    >
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <ReferralStats referralsCount={referrals.length} totalEarned={totalEarned} />
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <ReferralLinkCard referralLink={referralLink} copied={copied} onCopy={handleCopyLink} onShare={handleShare} />
      </motion.div>
