...
// Must match page protocol (HTTPS page requires wss:// connection)
const getBaseUrl = (): string => {
  // Server-side: not applicable for WebSocket (client-side only)
  if (typeof window === "undefined") {
    return process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8000/api";
  }

  // Client-side: use public URL and ensure protocol matches page
...
export const useGameWebSocket = (
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.event === "click") {
            ...
            // Invalidate queries to keep all pages in sync
  ...
  // Only update if the values are different and we haven't received WebSocket updates yet
  useEffect(() => {
    if (initialCoins > 0 && !hasReceivedUpdate.current) {
      setCoins(initialCoins);
    }
  }, [initialCoins]);

  useEffect(() => {
    if (initialEnergy > 0 && !hasReceivedUpdate.current) {
